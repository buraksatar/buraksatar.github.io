---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
excerpt: "CV of Burak Satar, Research Scientist at Singapore Management University; PhD in Computer Science from NTU; culturally-aware vision-language models."
redirect_from:
  - /resume
---

<a href="/files/cv.pdf" class="btn btn--primary">{% include icon.html name="file-pdf" %} Download CV (PDF)</a>

## Education

* **Ph.D. in Computer Science**, Nanyang Technological University (NTU), Singapore, 2020–2024
  * College of Computing and Data Science (CCDS, formerly SCSE); thesis: *Towards Semantic, Debiased and Moment Video Retrieval with Multi-modal Features* (defended Dec 2024)
  * Supervised by Prof Joo-Hwee Lim, Dr Hongyuan Zhu and Prof Hanwang Zhang; supported by the A\*STAR SINGA scholarship
* **M.Sc. in Electronics Engineering**, Uludağ University, Turkey, 2018
  * Supervised by Prof Ahmet Emir Dirik; thesis on vehicle detection and make-model classification
* **B.Sc. in Electronics Engineering**, Uludağ University, Turkey, 2014
  * Exchange semesters in Siena and Naples, Italy

## Work experience

* **Mar 2025 – Present: Research Scientist**, Singapore Management University (SMU), Singapore
  * Making vision-language models culturally aware, with reasoning across image, video, audio and text, starting with Southeast Asia; working with Prof Chong-Wah Ngo
* **Jan 2020 – Dec 2024: Doctoral Researcher (SINGA Scholar)**, Institute for Infocomm Research (I<sup>2</sup>R), A\*STAR, Singapore
  * Semantic, debiased and moment-level text-to-video retrieval with multi-modal features
* **Oct 2023 – Dec 2023: Visiting Researcher**, MaVi Research Group, University of Bristol, UK
  * Video corpus moment retrieval, with Dr Michael Wray in Dima Damen's group
* **Feb 2019 – Sep 2019: Machine Learning Engineer**, Turkish Technic (Turkish Airlines), Istanbul, Turkey
* **Sep 2016 – Mar 2017: Software Engineer** (Erasmus+ traineeship), University of Valencia, Spain
* **Dec 2014 – Sep 2015: Electronics Engineer**, TSV Energy (start-up), Turkey
* **Advisory**: machine learning advisor to two award-winning start-ups in London and Istanbul

## Awards & honours

* **Joint 3rd Place**, EPIC-Kitchens-100 Multi-Instance Retrieval Challenge, CVPR 2022
* **Finalist**, Three Minute Thesis (3MT) @ NTU, representing CCDS, 2022
* **SINGA Ph.D. Scholarship**, A\*STAR, 2020–2024
* **Student Travel Award**, European Neural Network Society (ICANN), 2018
* Oral presentations at ICIP 2021, ICCV Workshop 2021, CVPR Workshop 2023 and ICANN 2018

## Publications

See the [full publication list](/publications/) or my [Google Scholar profile](https://scholar.google.com/citations?user=qSgpP04AAAAJ).

## Academic service & activities

* Volunteer: ACM Web Conference 2024, NeurIPS 2021
* Programme Committee member & reviewer, ICANN 2019
* Summer schools: CIFAR DLRL 2022; PAISS 2021 (poster presentation)
* Teaching and mentoring: see [Teaching](/teaching/)

## Skills

* Python, PyTorch; training and evaluating multimodal models at scale
* Vision-language models, text-to-video retrieval, benchmark and dataset construction

## Languages

* Turkish (native), English (professional working proficiency), Italian & Spanish (elementary)
